...
// Strict hex color validation — prevents CSS injection via colorTheme fields
const HEX_RE = /^#[0-9a-fA-F]{3,8}$/
function safeColor(val, fallback) {
  return HEX_RE.test(String(val || '')) ? val : fallback
}

// Font allowlist — prevents CSS injection via fontFamily field
...
function safeFont(val) {
  return FONT_ALLOWLIST.includes(val) ? val : FONT_ALLOWLIST[0]
}

// Only allow http/https image URLs — blocks javascript: and data: schemes
function safeImageUrl(val, fallback) {
  if (!val) return fallback
  try {
    const u = new URL(val, location.href)
    return (u.protocol === 'http:' || u.protocol === 'https:') ? val : fallback
  } catch (_) {
    // relative path (e.g. /assets/placeholder.svg) is safe
    return /^\/[^/]/.test(val) ? val : fallback
  }
}
...
function buildHTMLFromJSON(d) {
  const raw = d.colorTheme || {}
  const colorTheme = {
    primary:   safeColor(raw.primary,   '#6366f1'),
    secondary: safeColor(raw.secondary, '#eef2ff'),
    accent:    safeColor(raw.accent,    '#4f46e5'),
    text:      safeColor(raw.text,      '#1e1b4b'),
    light:     safeColor(raw.light,     '#f5f3ff'),
  }
  const fontFamily = safeFont(d.fontFamily)
  const isCenter = d.layout === 'hero_center'

  const waPhone = (d.phoneNumber || '').replace(/[^0-9]/g, '')
  const waMsg = d.whatsappMessage ? `?text=${encodeURIComponent(d.whatsappMessage)}` : ''
  const waLink = waPhone ? escapeHTML(`https://wa.me/${waPhone}${waMsg}`) : '#'
  const heroImage = safeImageUrl(d.imageUrl, '/assets/placeholder.svg')
